...
* 作业执行类型( CloudJobExecutionType )有两种：常驻作业( DAEMON )，瞬时作业( TRANSIENT )。在[《Elastic-Job-Cloud 源码解析 —— 作业调度》](http://www.yunai.me?todo)详细解析。Elastic-Job-Cloud 独有，非常有趣。👍👍👍
* `beanName`, `applicationContext` 实现 Spring 启动方式作业。在[《Elastic-Job-Cloud 源码解析 —— 作业调度》](http://www.yunai.me?todo)有详细解析。
...
* ProducerManager，发布任务作业调度管理器。这是一个很重要的类，在[《Elastic-Job-Cloud 源码解析 —— 作业调度》](http://www.yunai.me?todo)详细解析。
